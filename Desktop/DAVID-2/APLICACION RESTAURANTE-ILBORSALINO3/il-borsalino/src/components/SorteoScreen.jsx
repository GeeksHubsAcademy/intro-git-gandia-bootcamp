...
class SorteosScreen extends Component {
    constructor(props) {
        super(props)
        this.state = {
           loading: true,
    ...
    fetchGiveaways = async () => {
        const res4 = await fetch('https://planetothebrain.com/ilborsalino/v1/sorteos/' + this.props.route.params.eventId.toString() + '/')
    ...
    render(){
        ...
        <>{this.state.loading ? null :<ScrollView>
            ...
            {this.determineIfActive(item) ? <TouchableOpacity style={styles.button} onPress={
                    ()=>{
